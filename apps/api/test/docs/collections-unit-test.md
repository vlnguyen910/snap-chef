...
const mockRecipe    = { id: 'recipe-uuid-1', title: 'Test Recipe', thumbnail_url: '...' };
...
it('should throw NotFoundException if current_user_id is invalid', async () => {
  ...
  await expect(service.getUserCollection(mockOwner.id, 'ghost-user')).rejects.toThrow(
...
it('should return "removed from" on second add (toggle behavior)', async () => {
  ...
  mockPrismaService.collection.findUnique.mockResolvedValue({ ...mockCollection, recipe: [] });
  mockRecipesService.findOne.mockResolvedValue(mockRecipe);
  const result1 = await service.addRecipe(mockCollection.id, mockRecipe.id, mockOwner.id);
  ...
  mockPrismaService.collection.findUnique.mockResolvedValue({
    ...mockCollection, recipe: [{ id: mockRecipe.id }],
  });
  const result2 = await service.addRecipe(mockCollection.id, mockRecipe.id, mockOwner.id);
